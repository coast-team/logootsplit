...
public class LogootSInsertion implements LogootSOperation {

    private LogootSElement element;
    private List content;

    private LogootSInsertion(LogootSElement el, List s) {
        element = el.clone();
        content = s;
    }

    public LogootSInsertion(LogootSElement lower, LogootSElement greater, List content, LogootSDocument doc, int siteId) {
    ...
    public LogootSInsertion(LogootSElement el, int offset, List content, LogootSDocument doc, int siteId) {
        this.content = content;
        List<LogootSIdentifier> idList = new ArrayList<LogootSIdentifier>();
        int i = 0;
        while (i < el.size() - 1) {
    ...
    public Operation clone() {
        return new LogootSInsertion(this.element, this.content);
    }
}